var firebaseConfig = {
  ...
  var listRef =storageRef.child("Student-resources/PureMathematics") ;


  const myForm = document.querySelector("#fileBtn");
  const storageRef1 = document.querySelector("#inpFile");
  var storageRef = firebase.storage().ref();
  var physicsStorage = storage.ref().child("Student-resources/PureMathematics");
...
*/
  ...
  // Variables
  var ref1  = "Student-resources/PureMathematics/"
  ...
  const storageRefOne = storage.ref('Student-resources/PureMathematics/');
  ...
  async function getFileInfo() {
    ...
    fileInfo.items.forEach(async item => { //Go through each file and render to DOM
      ...
      //Create path to file
      let pathToFile = `Student-resources/PureMathematics
      /${item.name}`;

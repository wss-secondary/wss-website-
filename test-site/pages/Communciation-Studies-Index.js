var firebaseConfig = {
  ...
  var listRef =storageRef.child("Student-resources/CommunicationStudies") ;
  

  const myForm = document.querySelector("#fileBtn");
  const storageRef1 = document.querySelector("#inpFile");
  var storageRef = firebase.storage().ref();
  var physicsStorage = storage.ref().child("Student-resources/CommunicationStudies");
  ...
  myForm.addEventListener('click',e=>{
      ...
      phystorageRef.put(storageRef1.files[0]).then(function(snapshot) {
...
*/
  ...
  // Variables
  var ref1  = "Student-resources/CommunicationStudies/"
  ...
  const storageRefOne = storage.ref('Student-resources/CommunicationStudies/');
  ...
  async function getFileInfo() {
    ...
    fileInfo.items.forEach(async item => { //Go through each file and render to DOM
      ...
      //Create path to file
      let pathToFile = `Student-resources/CommunicationStudies/${item.name}`;

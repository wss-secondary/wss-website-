var firebaseConfig = {
  ...
  var listRef =storageRef.child("Student-resources/Biology") ;
  

  const myForm = document.querySelector("#fileBtn");
  const storageRef1 = document.querySelector("#inpFile");
  var storageRef = firebase.storage().ref();
  var biologyStorage = storage.ref().child("Student-resources/Biology");
  ...
  myForm.addEventListener('click',e=>{
      ...
      biologyStorageRef.put(storageRef1.files[0]).then(function(snapshot) {
...
*/
  ...
  // Variables
  var ref1  = "Student-resources/Biology/"
  ...
  const storageRefOne = storage.ref('Student-resources/Biology/');
  ...
  async function getFileInfo() {
    ...
    fileInfo.items.forEach(async item => { //Go through each file and render to DOM
      ...
      //Create path to file
      let pathToFile = `Student-resources/Biology/${item.name}`;

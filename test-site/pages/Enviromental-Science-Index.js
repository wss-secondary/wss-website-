var firebaseConfig = {
  ...
  var listRef =storageRef.child("Student-resources/EnvironmentalStudies") ;
  

  const myForm = document.querySelector("#fileBtn");
  const storageRef1 = document.querySelector("#inpFile");
  var storageRef = firebase.storage().ref();
  var environmentalStudiesStorage = storage.ref().child("Student-resources/EnvironmentalStudies");

  myForm.addEventListener('click',e=>{
      e.preventDefault();
      let storageRef = firebase.storage().ref(storageRef1.files[0].name);
      console.log("here");
      console.log(storageRef1.files);
      
      console.log(storageRef1.files[0]);
      environmentalStudiesStorageRef.put(storageRef1.files[0]).then(function(snapshot) {
...
*/
  ...
  // Variables
  var ref1  = "Student-resources/EnvironmentalStudies/"
  ...
  const storageRefOne = storage.ref('Student-resources/EnvironmentalStudies/');
  ...
  async function getFileInfo() {
    ...
    fileInfo.items.forEach(async item => { //Go through each file and render to DOM
      ...
      //Create path to file
      let pathToFile = `Student-resources/EnvironmentalStudies/${item.name}`;

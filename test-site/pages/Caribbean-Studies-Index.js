var firebaseConfig = {
  ...
  var listRef =storageRef.child("Student-resources/CaribbeanStudies") ;
  

  const myForm = document.querySelector("#fileBtn");
  const storageRef1 = document.querySelector("#inpFile");
  var storageRef = firebase.storage().ref();
  var caribbeanStudiesStorage = storage.ref().child("Student-resources/CaribbeanStudies");
  ...
  myForm.addEventListener('click',e=>{
      ...
      caribbeanStudiesStorage.put(storageRef1.files[0]).then(function(snapshot) {
...
*/
  ...
  // Variables
  var ref1  = "Student-resources/CaribbeanStudies/"
  ...
  const storageRefOne = storage.ref('Student-resources/CaribbeanStudies/');
  ...
  async function getFileInfo() {
    ...
    fileInfo.items.forEach(async item => { //Go through each file and render to DOM
      ...
      //Create path to file
      let pathToFile = `Student-resources/CaribbeanStudies/${item.name}`;
